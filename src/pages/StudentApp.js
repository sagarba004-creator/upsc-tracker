...
export default function StudentApp({ user, onLogout }) {
  ...
  const unreadCount = feedback.filter(f => (f.created_date || '') > lastSeenFeedbackDate).length;
...
const CHAPTER_WEIGHTS = {}; // chapter-level pre/mains weight pills (populated if needed)
...
function SubjectsTab({ dashboard, user, onUpdate, gsSummary, showWeights=false }) {
  ...
  function TrendChart({ subjectName, examType }) {
    ...
    const ydata = (dashboard?.pyq_year_data || {})[subjectName] || {};
              ...
              {isOpen && (
                <div style={{ background: sec.light, display:'flex', flexDirection:'column', gap:8, padding:'10px 10px' }}>
                  {subjects.map((subj, idx) => {
                    ...
                    const hasPYQ = !!((dashboard?.pyq_year_data || {})[subj.subject]) && !['Philosophical','Socio-Politico-Economic'].includes(subj.subject) && !['Optional','Essay'].includes(subj.gs_paper);
                    return (
          ...
          {loading ? (
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:5 }}>
              {filtered
                .slice()
                .sort((a, b) => {
                  const ya = String(a.pyq_years).match(/\d{4}/);
                  const yb = String(b.pyq_years).match(/\d{4}/);
                  return (ya ? +ya[0] : 0) - (yb ? +yb[0] : 0);
                })
                .map((t, i) => {
                  const yr = String(t.pyq_years).match(/\d{4}/);
                  const year = yr ? yr[0] : '?';
                  const kw = (t.pyq_priority || '').trim();
                  return (
                    <div key={i} style={{
                      background:'#E8F5E9', border:'1px solid #A5D6A7',
                      borderRadius:6, padding:'5px 8px',
                      display:'flex', flexDirection:'column', gap:1,
                      flexBasis:'calc(50% - 3px)', flexGrow:0, flexShrink:0,
                      boxSizing:'border-box',
                    }}>
                      <span style={{ color:'#1B5E20', fontSize:11, fontWeight:800, lineHeight:1.2 }}>{year}</span>
                      {kw && <span style={{ color:'#2E7D32', fontSize:9, fontWeight:500, lineHeight:1.3 }}>{kw}</span>}
                    </div>
                  );
                })
              }
            ...
            <div key={f.id || i} className="card" style={{ marginBottom: 8, borderLeft: '3px solid #00695C' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 10, color: '#9CA3AF' }}>
                  🕐 {f.created_date ? new Date(f.created_date).toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric'}) : ''}
                </span>
